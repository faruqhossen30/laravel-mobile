...
@section('breadcrumb')
    <x-breadcrumb pageone="Product" pageoneRoute="{{ route('product.index') }}" pagetwo="Edit" />
@endsection
@section('content')
    @if ($errors->any())
                ...
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 lg:col-span-8 space-y-2 bg-white  dark:bg-gray-800 p-4 rounded-lg">
                        <div class="">
                            ...
                            <select id="category_ids" name="category_id"
                                class="select-category py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"  @if ($category->id == $product->category_id) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <x-form.input name="title" value="{{ $product->title }}" label="Product Title" />
                        <label for="description"
                            class="text-sm font-medium text-gray-500 dsark:text-gray-500">Description</label>
                        <div>
                            <textarea name="description" id="myeditorinstance" cols="30" rows="10">{{$product->description}}</textarea>
                            @error('description')
                    ...
                    <div class="col-span-12 lg:col-span-4 pt-1 space-y-2 bg-white  dark:bg-gray-800 p-4 rounded-lg">
                        ...
                        <x-form.card title="SEO Section">
                            <x-form.input name="meta_title" value="{{$product->meta_title}}" label="Meta Title" />
                            <x-form.textarea name="meta_description" value="{{$product->meta_description}}" label="Meta Description" />
                            <div>
                                <label for="meta_keywords"
                                    class="block text-sm font-medium mb-2 dark:text-white">meta_keyword</label>
                                <select id="meta_keywords" name="meta_keywords[]" value="{{$product->meta_keywords}}"
                        ...
                        <div>
                            <label for="slider" class="block text-sm font-medium mb-2 dark:text-white">
                                Slider</label>
                            <input class="dropify"type="file" id="slider" value="" name="sliders[]" multiple>
                        </div>
                    </div>
