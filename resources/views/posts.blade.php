                        ...
                        <div class=" bg-white border-b px-3 py-2">
                            ...
                            @foreach ($categories as $category)
                                <div class="flex items-center mb-4">
                                    <input @if(in_array($category->id, $categoryids)) checked @endif id="category-{{ $category->id }}" type="checkbox" value="{{ $category->id }}" name="category[]" onchange="this.form.submit()"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                    <label for="category-{{ $category->id }}" class="ms-2 text-sm font-medium text-gray-500 dark:text-gray-300">{{ $category->name }}</label>
                                </div>
                            @endforeach
                    ...
                    <div class="col-span-9">
                        <div class=" bg-white flex justify-between px-3 ">
                            <div class=" p-6 flex  space-x-2">
                                ...
                                <svg class="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                    viewBox="0 0 24 24">
                        ...
                        <div class="grid grid-cols-12 gap-3 py-2">
                            @foreach ($posts as $post)
                                <div class="col-span-4 border group bg-white rounded-lg ">
                                    <!-- Card -->
                                    <a class="group flex flex-col h-full border border-gray-200 hover:border-transparent hover:shadow-lg transition-all duration-300 rounded p-5 dark:border-neutral-700 dark:hover:border-transparent dark:hover:shadow-black/40"
                                        href="#">
                                        <p class="hidden">{{ $post->id }}</p>
                                        <div class="aspect-w-16 aspect-h-11">
                                            <img class="w-full object-cover rounded-xl" src="{{ asset('storage/' . $post->thumbnail) }}" alt="Image Description">
                                        </div>
                                        <div class="my-6">
                                            ...
                                            <p class="mt-5 text-gray-600 dark:text-neutral-400">
                                                <?php echo substr($post->description, 0, 150); ?>
                                            </p>
                                        </div>
                                    </a>
                                </div>
